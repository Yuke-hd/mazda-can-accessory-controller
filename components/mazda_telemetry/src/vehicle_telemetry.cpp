...
Reading<float> PublicationStore::speed_kph() const noexcept {
  return read_descriptor(&VehicleState::speed_kph, candidate::kEngineDataId,
                         ValidationStatus::Reference);
}

Reading<float> PublicationStore::engine_rpm() const noexcept {
  return read_descriptor(&VehicleState::engine_rpm, candidate::kEngineDataId,
  ...
  static const NotificationDescriptorTuple descriptors {
         ...
         signals::kFrontWiperPosition},
    {
      "test_front_wiper", &VehicleTelemetryService::test_front_wiper_channel_,
          ...
          candidate::kFrontWiperDefinition.confidence, vehicle_signals::SignalId {}
    }
#endif
  };
  return descriptors;
