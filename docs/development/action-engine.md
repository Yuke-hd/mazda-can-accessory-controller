...
`lib/action_engine` (#9, #10) turns generic signal notices and sampled
...
- `action_engine::ActionSink` receives `ActionCommand{ActionId, kind, level}`.
  ...
  dispatcher context (notified rules) or on the context that samples range
  rules.
...
| `DuplicateAction` | A state or range rule already drives this `ActionId` (state and range rules only). |
| `UnknownSignal` | The key is not in the catalog. |
| `UnsupportedCapability` | The signal cannot notify (state and event rules) or be read (range rules). |
...
| `CapacityExceeded` | 16 state/event rules, 8 range rules, or 4 sinks are already registered. |
...
State and range rules are level outputs, so each owns its `ActionId`. With
...
`NoData`, `Stale` and `Unavailable` are never actionable. State and range
rules treat any non-actionable reading, and a range rule also treats a failed
read, as off and emit `Deactivate`. This fail-off behavior is fixed.
...
event rule has no baseline, and a range rule has emitted nothing.
...
- Cadence: range rules are sampled, not notified. The application calls
  `ActionEngine::sample_range_rules()` at its own cadence, and each call reads
  every range rule's signal through `SignalProvider::read()`. This suits
  Read-only signals such as RPM. The signal layer has no polling worker and no
  synthetic notification. Range rules need no subscription.
  `sample_range_rules()` returns `InvalidState` while detached and `Ok`
...
- The provider delivers notices serially on its dispatcher context, and
  `sample_range_rules()` runs on the caller's context. The engine serializes
  ...
  taken, so a read never waits on a sink. Call `sample_range_rules()` from one
