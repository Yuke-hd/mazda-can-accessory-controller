...
// every registered sink in registration order, exactly once per sink.
// Registered sinks must outlive the fan-out's use.
class SinkFanOut final : public ActionSink {
public:
  static constexpr std::size_t kCapacity = 4;

  // Ok; DuplicateSink when `sink` is already registered; CapacityExceeded
  // when kCapacity sinks are registered.
  [[nodiscard]] ConfigStatus add(ActionSink &sink) noexcept;
  void execute(const ActionCommand &command) noexcept override;

private:
  [[nodiscard]] bool contains(const ActionSink &sink) const noexcept;
