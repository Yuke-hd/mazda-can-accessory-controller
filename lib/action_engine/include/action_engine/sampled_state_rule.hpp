...
// the reading is actionable and the condition holds"; a failed read or a
// non-actionable reading (NoData, Stale, Unavailable, a policy-rejected
// FreshnessUnverified, a mistyped or non-finite value) is fail-off.
// It emits Activate/Deactivate only when the output differs from the last
// emitted one; after reset() the last output is unknown, so the first sample
// always emits an explicit baseline.
class SampledStateRule final {
public:
  constexpr SampledStateRule() noexcept = default;
  constexpr SampledStateRule(const ResolvedCondition &condition, ActionId action) noexcept
